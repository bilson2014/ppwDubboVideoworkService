package com.paipianwang.service.video.service.impl;
...
public class VideoHandlerFacadeImpl implements VideoHandlerFacade {
	...
	public String converFile2Normal(final String fileId) {
		...
		if (StringUtils.isNotBlank(fileId)) {
			// 下载文件
			final InputStream ins = fastDFSBiz.downloadFile(fileId);
			try {
				if (ins != null) {
					...
					final String path = fastDFSBiz.uploadFile(targetFile, targetFile.getName());
